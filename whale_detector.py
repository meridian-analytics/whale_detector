...
def main(args):
    ...
    # Selected whale species to detect (e.g., bl, nw, or bh).
    species = args.species

    # Minimum prediction score required to classify a detection.
    score_thr = args.score_thr

    # CSV writing mode ('w' to overwrite or 'a' to append).
    mode_ = args.mode

    # Optional text file listing the WAV files to process.
    filelist = args.filelist

    # Number of WAV files assigned to each worker during multiprocessing.
    ...
    print(f"{'Project:':<28} {project_name}")
    print(f"{'Species:':<28} {species}")
    print(f"{'Input directory:':<28} {wav_files_path}")
    print(f"{'File list:':<28} {filelist if filelist else 'All WAV files'}")
    print(f"{'Output directory:':<28} {results_dir}")
    print(f"{'Project configuration:':<28} {project_config_path}")
    print(f"{'Whale model:':<28} {model_path}")
    print(f"{'Whale spectrogram config:':<28} {spec_config_path}")
    print(f"{'Vessel-noise model:':<28} {vnd_model_path}")
    print(f"{'Vessel spectrogram config:':<28} {vnd_spec_config_path}")
    print(f"{'Frequency band:':<28} {fmin}–{fmax} Hz")
    print(f"{'Detection threshold:':<28} {score_thr:.2f}")
    print(f"{'Batch size:':<28} {batch_size}")
    print(f"{'CSV write mode:':<28} {'Append' if mode_ == 'a' else 'Overwrite'}")
    ...
    # --------------------------------------------------------
    # Save the input parameters and processing log.
    # --------------------------------------------------------
        config_summary = (
        "\n"
        + "=" * 80 + "\n"
        ...
        + "=" * 80 + "\n"
        + f"{'Project:':<28} {project_name}\n"
        + f"{'Species:':<28} {species}\n"
        + f"{'Input directory:':<28} {wav_files_path}\n"
        + f"{'File list:':<28} {filelist if filelist else 'All WAV files'}\n"
        + f"{'Output directory:':<28} {results_dir}\n"
        + f"{'Project configuration:':<28} {project_config_path}\n"
        + f"{'Whale model:':<28} {model_path}\n"
        + f"{'Whale spectrogram config:':<28} {spec_config_path}\n"
        + f"{'Vessel-noise model:':<28} {vnd_model_path}\n"
        + f"{'Vessel spectrogram config:':<28} {vnd_spec_config_path}\n"
        + f"{'Frequency band:':<28} {fmin}–{fmax} Hz\n"
        + f"{'Detection threshold:':<28} {score_thr:.2f}\n"
        + f"{'Batch size:':<28} {batch_size}\n"
        + f"{'CSV write mode:':<28} {'Append' if mode_ == 'a' else 'Overwrite'}\n"
        + "=" * 80 + "\n"
    )

    # Save to a log file.
    log_file = results_dir / f"{project_name}_processing_configuration_log.txt"
...
if __name__ == "__main__":
    ...
    default=4,
    help=(
        "Number of WAV files processed by each worker during "
        "multiprocessing. Larger values may improve throughput "
        "but increase memory usage (default: 4)."
    )
    )
